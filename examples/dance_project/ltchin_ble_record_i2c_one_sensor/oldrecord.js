// PARAMS TO CHANGE
const FILENAME = "Demo" // Will auto add timestamp
// Sensor 0 = Below Knee, Sensor 3 = Above Knee
...
onButton.addEventListener('click', () => writeLED(1));
offButton.addEventListener('click', () => writeLED(0));
...
function writeLED(value) {
    BLE.write(ledCharacteristic, value)
...
function plotData(values) {
    ...
    switch(Number(values[0])) {
        ...
        case 3:
            ...
            break;
        default:
            throw new Error("Undefined value from the bluetooth server! " + value[0])
    }
    chart.data.datasets[series].data.push({x: esp32_time, y: pressure});
    chart.data.datasets[2].data.push({x: esp32_time, y: trigger});

    if (chart.data.datasets[2].data.length > MOVING_WINDOW) {
